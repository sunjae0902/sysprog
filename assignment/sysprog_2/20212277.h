...
void edit_ro_data(int32_t fd, Elf64_Shdr sh);
...
void print_section_headers(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[])
{
	...
	for(i=0; i<eh.e_shnum; i++) {
		if(!strncmp((sh_str + sh_table[i].sh_name), ".rodata", 7))
		{
			...
			sh_table[i].sh_flags |= SHF_WRITE; // give a permission to write
			edit_ro_data(fd,sh_table[i]);
            sh_table[i].sh_flags &= ~SHF_WRITE; // revoke a permission to write
		} 
	}
...
void edit_ro_data(int32_t fd, Elf64_Shdr sh) {
    int pos = 0;
    char *target = "software";
    char *newStr = "hackers!";
    size_t newSize = strlen(newStr) + 1;

    char *orgStr = read_section(fd, sh);
    for(size_t i = 0; i < sh.sh_size; i++){
	int j;
	for(j = 0; j < strlen(target); j++)
	    if(orgStr[i+j] != target[j]) break;
	if(j == strlen(target)) pos = i;
}
    // Calculate the offset from the start of the section
    off_t offset = sh.sh_offset + pos;
    assert(lseek(fd, offset, SEEK_SET) != (off_t)-1);
    // Write the new string
    assert(write(fd, newStr, newSize) == newSize);
